...
# ---------------------------------------------------------
# This allows the HTML to access files in the 'static' folder
app.mount("/static", StaticFiles(directory="static"), name="static")


# ---------------------------------------------------------
# 2. API ROUTES (Chatbot Logic)
# ---------------------------------------------------------
@app.post("/")
async def handle_request(request: Request):
    ...
    if intent in intent_handler_dict:
        ...
        return JSONResponse(content={"fulfillmentText": f"Intent {intent} not handled in backend."})
...
def track_order(parameters: dict, session_id: str):
    try:
        order_id_param = parameters.get('order_id') or parameters.get('number') or parameters.get('order-id')
        if order_id_param:
            ...
            fulfillment_text = "I couldn't find an order ID. Please specify it."
